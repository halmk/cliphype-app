...
export default {
  ...
  async mounted() {
    ...
    this.$cliphype.apiURL = `${this.$config.apiURL}/api`
    this.$cliphype.token = this.$cookies.get('jwt')
    await this.getStreamerId()
    await this.getAutoClips()
  ...
  methods: {
    ...
    async getAutoClips() {
      ...
      try {
        const response = await this.$cliphype.getAutoClips({
          streamer: this.streamer,
          started_at: this.dateRange[0],
          ended_at: this.dateRange[1],
        })
        console.log(response)
        this.autoclips = response.data.autoclips
        const autoclipIDs = this.autoclips.map((obj) => obj.clipID)
        await this.getClips(autoclipIDs)
        this.mergeHypes()
        this.clips.sort((a, b) => {
          if (a.view_count < b.view_count) {
            return 1
          } else if (parseInt(a.hype) <= parseInt(b.hype)) {
            return 1
          } else {
            return -1
          }
    ...
    mergeHypes() {
      this.clips = this.clips.map((obj) => {
        const hype = this.autoclips.find(
          (autoclip) => autoclip.clipID === obj.id
        ).hype
        obj.hype = hype.toFixed(2)
        return obj
      })
    ...
    async getClips(clipIDs) {
      try {
        const response = await this.$twitch.app.getClipsById(clipIDs.join(','))
        const data = response.data.response.data
