<template>
  <div>
    ...
    <v-dialog v-model="clipDialog" :width="bkPoint.dialogWidth" dark hide-overlay>
      <iframe
        :src="embedClipUrl"
...
export default {
  ...
  computed: {
    ...
    shownClips () {
      const shownClips = []
      for(let i=this.page*this.size; i < (this.page+1)*this.size; i++) {
  ...
  methods: {
    ...
    getStreamerId () {
      this.$twitch.getUserId(this.streamer)
        .then((response) => {
    ...
    getClips () {
      console.log("getClips params: ", this.streamerId, this.dateRange[0], this.dateRange[1]);
      this.$twitch.getClips(this.streamerId, this.dateRange[0], this.dateRange[1])
        .then((response) => {
    ...
    getAfterClips () {
      ...
      this.$twitch.getAfterClips(this.streamerId, this.dateRange[0], this.dateRange[1], this.clipsAfter)
        .then((response) => {
          // console.log(response);
          const data = response.data.response.data;
